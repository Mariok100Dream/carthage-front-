...
const  CardDescriptionTranslator = (props) => {
  ...
  useEffect(()=>{
    ...
    for(let i=0;i<props.data.length;i++){
        let f = {
          from : props.data[i].children[1].text,
          to :"", 
          id :uuidv4()
        }
        result.push(f)
  ...
  let [translation_language,setTranslatedLanguage] = useState(
    language_array
            .filter(el => el.key !=props.data[0].children[1].lang )[0].key
  )
  ...
  let handleChangeLanguage = async(value) =>{
    ...
    if(value!=="none"){
      ...
      for(let i=0;i<rowsTranslation.length;i++){
        let response_data =""
        try{
          response_data  = await axios.post(`https://api.mymemory.translated.net/get?q=${rowsTranslation[j].from}&langpair=${props.data[i].children[1].lang}|${value}`)
      ...
      for(let i=0;i<props.data.length;i++){
        props.data[i].children[1].text = result[i].to
        props.data[i].children[1].lang  = translation_language
      } 
      setLoader(false)
...
{!Loader ?
 <>
 {/* Select language */}
 <h4>{t("selectedLanguageIs")} { props.data[0].children[1].lang }</h4>
 <Box sx={{ minWidth: 120 }} style={{marginTop:"2%"}}>
      <FormControl fullWidth>
        ...
        >
            {language_array
            .filter(el => el.key !=props.data[0].children[1].lang )
            .map((el)=>(
...
export default CardDescriptionTranslator

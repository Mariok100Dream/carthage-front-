export let updateSlider = (
  ...
  let data = characters.filter(
    (el) => el.status != "New Order" && el.id == section_id
  )[0];
  let all = itemsData;
  //write some sort of function that comminicated between static changes and
  ...
  let ch_css = section_css(searcher, hasSlider, searcher.section_css);

  return { final_html, ch_css };
};

let section_css = (searcher, hasSlider, css) => {
  let ch_css = css;
  if (hasSlider) {
    ch_css = css
      .replace("$color_arrow_left", searcher.global_style[5]["color"])
      .replace(
        "$color_hover_arrow_left",
        searcher.global_style[5]["hover_color"]
      )
      .replace(
        "$background_color_arrow_left",
        searcher.global_style[5]["background_color"]
      )
      .replace(
        "$background_hover_color_arrow_left ",
        searcher.global_style[5]["background_hover_color"]
      )
      ...
      .replace(
        "$color_hover_arrow_right",
        searcher.global_style[6]["hover_color"]
      )
      .replace(
        "$background_color_arrow_right",
        searcher.global_style[6]["background_color"]
      )
      .replace(
        "$background_hover_color_arrow_right ",
        searcher.global_style[6]["background_hover_color"]
      )
...
export let updateSliderForHistory = (
  characters,
  section_id,
  searcher,
  hasSlider,

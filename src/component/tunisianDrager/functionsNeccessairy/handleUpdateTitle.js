import {updateTitleJS} from "./UpdateTitle"



export let handleUpdateTitle = (characters,global_data,global_object,section_id) =>{
  let itemsData = global_data.data_team
    ...
    let searcher = characters.filter(el => el.id === section_id)[0]
      ...
      if(global_object.title_text!=""){
        data.title.text = global_object.title_text
      }
       if(global_object.title_color!=""){
      
        data.title.color = global_object.title_color

      
      }
      if(global_object.title_hover_color!=""){
        data.title.hover_color = global_object.title_hover_color
       
      }
       
      if(global_object.title_font_family!=""){
        data.title.font_family = global_object.title_font_family
 
      }
      if(global_object.title_font_size!=""){
        data.title.font_size = global_object.title_font_size

      }
      if(global_object.title_padding_top!=""){
        data.title.padding_top = global_object.title_padding_top

      }
      if(global_object.title_padding_bottom!=""){
        data.title.padding_bottom = global_object.title_padding_bottom

      }
      if(global_object.title_padding_left!=""){
        data.title.padding_left = global_object.title_padding_left

      }
      if(global_object.title_padding_right!=""){
        data.title.padding_right = global_object.title_padding_right

      }
      if(global_object.title_margin_top!=""){
        data.title.margin_top = global_object.title_margin_top
       
      }

      if(global_object.title_margin_bottom!=""){
        data.title.margin_bottom = global_object.title_margin_bottom 
 
      }
      if(global_object.title_margin_left!=""){
        data.title.margin_left = global_object.title_margin_left
    
      }
      if(global_object.title_margin_right!=""){
        data.title.margin_right = global_object.title_margin_right
       
      }
      if(global_object.alignmentTitleAlign!=""){
        data.title.text_align = global_object.alignmentTitleAlign
